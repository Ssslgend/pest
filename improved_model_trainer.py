...
class ImprovedBiLSTMGCNModel(nn.Module):
    """改进的BiLSTM+GCN融合模型"""

    def __init__(self, input_size, hidden_size=64, num_classes=4, dropout=0.3):
        super(ImprovedBiLSTMGCNModel, self).__init__()
...
class ImprovedModelTrainer:
    ...
    def create_data_loaders(self, batch_size=32):
        ...
        # 创建数据集
        train_dataset = ImprovedDataset(self.config.train_data)
        val_dataset = ImprovedDataset(self.config.val_data)
        test_dataset = ImprovedDataset(self.config.test_data)
    ...
    def train_model(self, num_epochs=200, learning_rate=0.001):
        ...
        class_weights = self.calculate_class_weights(
            [sample['target'] for sample in ImprovedDataset(self.config.train_data).samples]
        )
    ...
    def evaluate_model(self, data_loader, dataset_name="测试"):
        ...
        results_path = f'results/improved_predictions/{dataset_name.lower()}_results.csv'
    ...
    def create_confusion_matrix(self, targets, predictions, dataset_name):
        """创建混淆矩阵"""
        os.makedirs('results/improved_visualizations', exist_ok=True)
        ...
        plt.ylabel('实际')
        plt.tight_layout()
        plt.savefig(f'results/improved_visualizations/{dataset_name.lower()}_confusion_matrix.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def create_training_curves(self):
        ...
        plt.tight_layout()
        plt.savefig('results/improved_visualizations/training_curves.png', dpi=300, bbox_inches='tight')
        plt.close()

        print("训练曲线保存到: results/improved_visualizations/training_curves.png")
    ...
    def save_model(self):
        ...
        # 保存完整模型信息
        model_info = {
            'model_type': 'ImprovedBiLSTMGCNModel',
            'input_size': self.config.NUM_FEATURES,
            'hidden_size': 64,
            'num_classes': self.config.NUM_CLASSES,
    ...
    def generate_comprehensive_report(self, test_metrics):
        ...
        report = {
            'training_date': datetime.now().isoformat(),
            'dataset_info': {
                'total_counties': self.config.train_data['County'].nunique() + self.config.val_data['County'].nunique() + self.config.test_data['County'].nunique(),
            ...
            'improvements': {
                'data_split_strategy': 'stratified_time_series_split_with_test_as_val',
                'increased_samples': {
                    'val_samples': len(self.val_loader.dataset),
                    'test_samples': len(self.test_loader.dataset),
                    'note': '测试集使用验证集数据，增加30%样本用于验证'
                },
                'balance_improvement': 'stratified_sampling'
            }
        }

        # 保存报告
        report_path = os.path.join(self.config.RESULTS_DIR, 'improved_model_report.json')
