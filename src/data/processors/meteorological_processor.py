#!/usr/bin/env python3
"""
使用真实的栅格气象数据整合山东省美国白蛾发病数据
基于县边界从栅格文件中提取真实的气象数据
"""
...
class RealMeteoIntegrator:
    def __init__(self, excel_path, shp_path, raster_data_dir, output_dir="datas/shandong_pest_data"):
        """
        初始化真实气象数据整合器

        Args:
            excel_path: Excel发病数据文件路径
            shp_path: 山东省县边界Shapefile路径
...
def main():
    ...
    # 创建整合器
    integrator = RealMeteoIntegrator(excel_path, shp_path, raster_data_dir, output_dir)
