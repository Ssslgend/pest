...
class ImprovedDataProcessor:
    ...
    def load_improved_datasets(self):
        ...
        # 加载数据
        train_data = pd.read_csv(os.path.join(base_path, 'improved_train.csv'))
        val_data = pd.read_csv(os.path.join(base_path, 'improved_val.csv'))
        test_data = pd.read_csv(os.path.join(base_path, 'improved_test.csv'))
...
def train_sf_bilstm_improved():
    """使用改进数据训练SF-BiLSTM模型"""
    print("=" * 60)
    print("使用改进数据训练SF-BiLSTM模型")
    print("=" * 60)
    ...
    # 加载数据
    train_data, val_data, test_data = processor.load_improved_datasets()
        ...
        try:
            ...
            with torch.no_grad():
                for data, _ in val_loader:
                    data = data.to(device)
                    output = model(data)
                    val_probs.extend(torch.sigmoid(output).cpu().numpy().flatten())
            val_auc = roc_auc_score(val_targets_np, val_probs)
        except:
    ...
    # 保存结果
    os.makedirs('results/sf_bilstm_improved_data', exist_ok=True)
    with open('results/sf_bilstm_improved_data/training_results.json', 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=4)

    # 保存模型
    torch.save(model.state_dict(), 'results/sf_bilstm_improved_data/best_model.pth')

    # 保存训练历史
    history_df = pd.DataFrame(train_history)
    history_df.to_csv('results/sf_bilstm_improved_data/training_history.csv', index=False)

    print(f"\n训练完成！结果已保存到: results/sf_bilstm_improved_data/")

    # 与之前结果对比
    print(f"\n性能对比:")
    print(f"原始数据 (8.1%发生率)   - F1: 0.1131, AUC: 0.9069")
    print(f"平衡数据 (修改后)       - F1: 0.1633, AUC: 0.8495")
    print(f"改进数据 (22.4%发生率)  - F1: {f1:.4f}, AUC: {auc:.4f}")

    improvement_vs_original = f1 - 0.1131
    improvement_vs_balanced = f1 - 0.1633

    print(f"\nF1改善:")
    print(f"相比原始数据: {improvement_vs_original:+.4f} ({improvement_vs_original/0.1131*100:+.1f}%)")
    print(f"相比平衡数据: {improvement_vs_balanced:+.4f} ({improvement_vs_balanced/0.1633*100:+.1f}%)")

    return results, model

if __name__ == "__main__":
    results, model = train_sf_bilstm_improved()