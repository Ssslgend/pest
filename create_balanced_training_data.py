...
class BalancedTrainingDataGenerator:
    """生成平衡的训练数据（只考虑活跃月份）"""
    ...
    def filter_active_months(self, df):
        """只保留美国白蛾活跃月份的数据"""
        print("筛选美国白蛾活跃月份...")

        # 美国白蛾活跃月份：5-10月（幼虫期到成虫期）
        active_months = [5, 6, 7, 8, 9, 10]

        # 筛选活跃月份
        df_active = df[df['month'].isin(active_months)].copy()

        print(f"筛选前数据: {len(df)} 条记录")
        print(f"筛选后数据: {len(df_active)} 条记录")
        print(f"保留比例: {len(df_active)/len(df):.2%}")

        # 分析活跃月份的数据分布
        print(f"\n活跃月份数据分布:")
        print(f"县区数: {df_active['county_name'].nunique()}")
        print(f"Has_Occurrence分布:")
        print(df_active['Has_Occurrence'].value_counts())
        print(f"活跃月份发生率: {df_active['Has_Occurrence'].mean():.4f}")
    ...
    def merge_data(self, df_occurrence, df_meteo):
        """合并发病数据和气象数据"""
        print("合并发病数据和气象数据...")
    ...
    def add_engineered_features(self, df):
        ...
        def get_activity_level(row):
            ...
            elif month == 7:  # 二龄三龄幼虫期
                return 5 if temperature > 22 else 4
            elif month == 8:  # 四龄五龄幼虫期（危害高峰）
                return 6 if temperature > 25 else 5
            elif month == 9:  # 老熟幼虫和化蛹期
                return 4 if temperature > 20 else 3
            elif month == 10:  # 成虫期
                return 2 if temperature > 15 else 1
            else:
        ...
        df['Moth_Activity_Level'] = df.apply(get_activity_level, axis=1)

        print(f"添加工程特征后: {df.shape}")
        return df

    def create_balanced_datasets(self, df):
        """创建平衡的数据集"""
        print("创建平衡的数据集...")

        # 选择特征列
        available_columns = df.columns.tolist()
        print(f"可用列名: {available_columns[:20]}...")
        ...
        print(f"正负样本比例: 1:{(len(df_clean) - df_clean['Has_Occurrence'].sum())/df_clean['Has_Occurrence'].sum():.1f}")
        ...
        # 保存数据集
        train_path = os.path.join(self.output_dir, "balanced_train.csv")
        val_path = os.path.join(self.output_dir, "balanced_val.csv")
        test_path = os.path.join(self.output_dir, "balanced_test.csv")
        complete_path = os.path.join(self.output_dir, "balanced_complete_data.csv")
        ...
        # 保存标准化器
        scaler_path = os.path.join(self.output_dir, "balanced_scaler.joblib")
    ...
    def generate_statistics(self, train_data, val_data, test_data, feature_columns):
        ...
        stats = {
            ...
            "dataset_info": {
                ...
                "val_years": [2022],
                "test_years": [2023],
                "month_filter": "May-October (Active Months)"
            },
            "label_distribution": {
                "train": {
            ...
            "improvement": {
                "old_train_occurrence_rate": 0.0405,  # 原始训练集发生率
                "new_train_occurrence_rate": float(train_data['Has_Occurrence'].mean()),
                "rate_improvement": float(train_data['Has_Occurrence'].mean() - 0.0405),
                "old_imbalance_ratio": 23.7,  # 原始不平衡比例
                "new_imbalance_ratio": float((len(train_data) - train_data['Has_Occurrence'].sum()) / train_data['Has_Occurrence'].sum())
            },
            "feature_columns": feature_columns
        }

        # 保存统计报告
        stats_path = os.path.join(self.output_dir, "balanced_data_statistics.json")
        ...
        print(f"活跃月份: {stats['data_info']['active_months']}")

        print(f"\n训练集 ({stats['data_info']['train_samples']} 样本):")
        print(f"  发生率: {stats['label_distribution']['train']['occurrence_rate']:.4f}")
        print(f"  不平衡比例: 1:{stats['label_distribution']['train']['imbalance_ratio']:.1f}")
        ...
        print(f"  不平衡比例: 1:{stats['label_distribution']['val']['imbalance_ratio']:.1f}")

        print(f"\n测试集 ({stats['data_info']['test_samples']} 样本):")
        print(f"  发生率: {stats['label_distribution']['test']['occurrence_rate']:.4f}")
        print(f"  不平衡比例: 1:{stats['label_distribution']['test']['imbalance_ratio']:.1f}")

        print(f"\n改进效果:")
        print(f"  训练集发生率: {stats['improvement']['old_train_occurrence_rate']:.1%} → {stats['improvement']['new_train_occurrence_rate']:.1%}")
        print(f"  不平衡比例: 1:{stats['improvement']['old_imbalance_ratio']:.1f} → 1:{stats['improvement']['new_imbalance_ratio']:.1f}")
    ...
    def run(self):
        ...
        try:
            ...
            # 2. 筛选活跃月份
            df_active = self.filter_active_months(df_occurrence)

            # 3. 加载气象数据
            df_meteo = self.load_meteorological_data()

            # 4. 合并数据
            merged_df = self.merge_data(df_active, df_meteo)

            # 5. 添加工程特征
            merged_df = self.add_engineered_features(merged_df)

            # 6. 创建平衡数据集
            train_data, val_data, test_data, feature_columns, scaler = self.create_balanced_datasets(merged_df)
...
if __name__ == "__main__":
    generator = BalancedTrainingDataGenerator()
