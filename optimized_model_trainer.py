...
class OptimizedDataset(Dataset):
    ...
    def __init__(self, data, sequence_length=2, feature_selector=None):
        self.data = data
        ...
        if len(self.samples) > 0:
            ...
            if self.feature_selector is not None:
                ...
                self.selected_features = np.ones(all_features.shape[1], dtype=bool)
...
class OptimizedBiLSTMGCNModel(nn.Module):
    """优化的BiLSTM+GCN融合模型"""

    def __init__(self, input_size, hidden_size=128, num_classes=4, dropout=0.4):
        super(OptimizedBiLSTMGCNModel, self).__init__()
        ...
        self.bilstm = nn.LSTM(
            ...
            batch_first=True,
            bidirectional=True,
            dropout=0.3 if hidden_size > 1 else 0
        )
    ...
    def forward(self, x):
        ...
        # 全局平均池化
        lstm_mean = torch.mean(lstm_out, dim=1)  # [batch, hidden*2]
        attn_mean = torch.mean(attn_out, dim=1)  # [batch, hidden]

        # 融合所有特征
        combined = torch.cat([lstm_last, lstm_mean, attn_last, original_last], dim=-1)  # [batch, hidden*4]
...
class OptimizedModelTrainer:
    ...
    def __init__(self):
        ...
        # 打印数据分割摘要
        self.config.print_data_split_summary()
    ...
    def setup_feature_selection(self):
        ...
        # 准备训练数据用于特征选择
        X_train = self.config.train_data[self.config.ALL_FEATURES]
        y_train = self.config.train_data['Severity_Level']

        # 使用互信息选择重要特征
        self.feature_selector = SelectKBest(mutual_info_classif, k=25)
        self.feature_selector.fit(X_train, y_train)

        selected_features = np.array(self.config.ALL_FEATURES)[self.feature_selector.get_support()]
        print(f"原始特征数: {len(self.config.ALL_FEATURES)}")
    ...
    def create_data_loaders(self, batch_size=16):
        ...
        # 创建数据集
        train_dataset = OptimizedDataset(self.config.train_data, feature_selector=self.feature_selector)
        val_dataset = OptimizedDataset(self.config.val_data, feature_selector=self.feature_selector)
        test_dataset = OptimizedDataset(self.config.test_data, feature_selector=self.feature_selector)
    ...
    def create_model(self):
        ...
        # 获取特征选择后的输入大小
        input_size = sum(self.feature_selector.get_support())
        hidden_size = 128
        num_classes = self.config.NUM_CLASSES

        self.model = OptimizedBiLSTMGCNModel(
            input_size=input_size,
            hidden_size=hidden_size,
    ...
    def train_model(self, num_epochs=300, learning_rate=0.0005):
        """训练模型"""
        print(f"\n=== 开始训练优化模型 ===")
        ...
        class_weights = self.calculate_class_weights(
            [sample['target'] for sample in OptimizedDataset(self.config.train_data, feature_selector=self.feature_selector).samples]
        )
        class_weights_tensor = torch.FloatTensor(class_weights).to(self.device)
        criterion = nn.CrossEntropyLoss(weight=class_weights_tensor, label_smoothing=0.1)  # 添加标签平滑
    ...
    def evaluate_model(self, data_loader, dataset_name="测试"):
        ...
        results_path = f'results/optimized_predictions/{dataset_name.lower()}_results.csv'
    ...
    def create_confusion_matrix(self, targets, predictions, dataset_name):
        """创建混淆矩阵"""
        os.makedirs('results/optimized_visualizations', exist_ok=True)
        ...
        plt.ylabel('实际')
        plt.tight_layout()
        plt.savefig(f'results/optimized_visualizations/{dataset_name.lower()}_confusion_matrix.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def create_training_curves(self):
        ...
        plt.tight_layout()
        plt.savefig('results/optimized_visualizations/training_curves.png', dpi=300, bbox_inches='tight')
        plt.close()

        print("训练曲线保存到: results/optimized_visualizations/training_curves.png")

    def save_model(self):
        """保存模型"""
        os.makedirs('results/optimized_models', exist_ok=True)

        # 保存模型权重
        model_path = os.path.join('results/optimized_models', 'optimized_bilstm_gcn_model.pth')
        torch.save(self.model.state_dict(), model_path)

        # 保存完整模型信息
        model_info = {
            'model_type': 'OptimizedBiLSTMGCNModel',
            'input_size': sum(self.feature_selector.get_support()),
            'hidden_size': 128,
            'num_classes': self.config.NUM_CLASSES,
            ...
            'selected_features': list(np.array(self.config.ALL_FEATURES)[self.feature_selector.get_support()]),
            'feature_categories': self.config.get_feature_categories()
        }

        info_path = os.path.join('results/optimized_models', 'optimized_model_info.json')
    ...
    def generate_comprehensive_report(self, test_metrics):
        ...
        report = {
            ...
            'optimization_applied': {
                ...
                'feature_count': sum(self.feature_selector.get_support()),
                'robust_scaling': True,
                'label_smoothing': True,
                'increased_model_capacity': True,
        ...
        # 保存报告
        os.makedirs('results/optimized_predictions', exist_ok=True)
        report_path = os.path.join('results/optimized_predictions', 'optimized_model_report.json')
...
def main():
    ...
    # 创建数据加载器
    class_weights = trainer.create_data_loaders(batch_size=16)

    # 创建模型
    trainer.create_model()

    # 训练模型
    trainer.train_model(num_epochs=300, learning_rate=0.0005)
    ...
    print(f"验证集F1分数: {val_metrics['f1_weighted']:.4f}")
