...
def create_bilstm_data_loaders(X_data, y_data, batch_size=32):
    ...
    data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=True if "train" in str(data_loader) else False) # 根据是否是训练集决定是否shuffle
...
def train_and_evaluate_bilstm(X_train, X_test, y_train, y_test, input_size):
    ...
    # 创建数据加载器 (传入训练和测试数据)
    train_loader = create_bilstm_data_loaders(X_train, y_train, batch_size=32)
    test_loader = create_bilstm_data_loaders(X_test, y_test, batch_size=32)
