...
class NewDataProcessor:
    ...
    def load_new_datasets(self):
        ...
        # 检查标签分布
        print(f"\n标签分布:")
        print(f"训练集 - 发生率: {train_data['Has_Occurrence'].mean():.4f}")
        print(f"验证集 - 发生率: {val_data['Has_Occurrence'].mean():.4f}")
        print(f"测试集 - 发生率: {test_data['Has_Occurrence'].mean():.4f}")
...
def train_sf_bilstm_newdata():
    """使用新数据训练SF-BiLSTM模型"""
    print("=" * 60)
    print("使用新数据训练SF-BiLSTM模型")
    print("=" * 60)
    ...
    # 加载数据
    train_data, val_data, test_data = processor.load_new_datasets()
    ...
    for epoch in range(config['epochs']):
        ...
        try:
            ...
            val_acc = accuracy_score(val_targets_np, (val_preds_np > 0.5).astype(int))
        except:
            val_auc = 0.5
            val_acc = 0.5
    ...
    if best_model_state is not None:
        ...
        print(f"加载最佳模型，验证AUC: {best_val_auc:.4f}")
    ...
    # 保存结果
    os.makedirs('results/sf_bilstm_newdata', exist_ok=True)
    with open('results/sf_bilstm_newdata/training_results.json', 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=4)

    # 保存模型
    torch.save(model.state_dict(), 'results/sf_bilstm_newdata/best_model.pth')

    # 保存训练历史
    history_df = pd.DataFrame(train_history)
    history_df.to_csv('results/sf_bilstm_newdata/training_history.csv', index=False)

    print(f"\n训练完成！结果已保存到: results/sf_bilstm_newdata/")

    return results, model

if __name__ == "__main__":
    results, model = train_sf_bilstm_newdata()